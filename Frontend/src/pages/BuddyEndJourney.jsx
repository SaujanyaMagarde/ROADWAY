...
import BuddyFinalRide from '../components/BuddyFinalRide.jsx';

function BuddyEndJourney({ status = null, details = null }) {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const gotodestRef = useRef(null);
  const ridedetails = details;
  const user = useSelector((state) => state.auth.userdata);
  const [isFullHeight, setIsFullHeight] = useState(true);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  const [routePolyline, setRoutePolyline] = useState(null);
  const [ride, setRide] = useState(details);
...
export default BuddyEndJourney;
