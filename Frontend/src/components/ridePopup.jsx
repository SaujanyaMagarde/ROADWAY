...
function RidePopup({setridePopup,setConformRide}) {
  return (
    <div>
      <h3
      ...
      <h3 className='text-2xl mt-5 mb-5 font-bold ' >New Ride Available! </h3>
      ...
      <div className='w-full'>
        ...
        <button 
        onClick={()=>(setConformRide(true))}
        className='w-full mt-5 bg-green-400 font-semibold text-white p-2 rounded-lg '>Accept</button>
        <button 
        onClick={()=>(setridePopup(false))}
        className='w-full mt-5  bg-gray-400 font-semibold text-white p-2 rounded-lg '>Ignore!</button>
      </div>
    </div>
  )
}
